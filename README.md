...
- 재경기: 줄다리기·땅따먹기 모두 `GAME_REMATCH_REQUEST`로 같은 세션 ID를 유지한 채 재시작할 수 있습니다. 상대가 먼저 신청하면 `GAME_REMATCH_NOTICE`로 알림을 받습니다.
- Land Grab 온라인: 10x10 보드를 실시간으로 양쪽에 브로드캐스트합니다. 콤보 10 이상이면 상대 타일을 바로 빼앗을 수 있고, 스플래시/보호막/콤보가드·먹물/EMP/혼란 아이템이 발동합니다. 내 시점 기준으로 보드 색상과 보호막이 렌더링됩니다.
- 줄다리기 싱글: LandGrab 사운드를 재사용한 효과음/BGM이 추가돼 시작/정답/패배/무승부에 맞춰 재생됩니다.
...
4. 재경기
   - 경기 종료 오버레이 또는 하단 `재경기` 버튼을 누르면 `GAME_REMATCH_REQUEST`가 전송됩니다. 양쪽 모두 누르면 동일한 세션 ID로 타이머/보드/밧줄이 초기화되며, 응답 대기 30초 후 자동 닫힙니다.
5. 기본 포트는 `7777`이며 외부 접속 시 방화벽 예외가 필요할 수 있습니다.
...
- 현재 난이도별 텍스트 파일은 포함되어 있지 않습니다. 필요하면 `words/` 아래에 `ko_easy.txt` 등 새 파일을 추가한 뒤 JSON으로 변환해 사용하세요.
