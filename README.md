...
- 현재 제공 게임
  1. **줄다리기 타자 대전 (TugOfWarGame)** – 단어는 70% 일반 / 15% 버프 / 15% 트랩, 싱글/온라인 모두 지원.
  2. **성 지키기 (CastleDefenseGame)** – 난이도별 웨이브를 타이핑으로 방어하는 디펜스 모드.
  3. **땅따먹기 Land Grab (LandGrabGame)** – 10x10 보드에서 단어를 맞춰 타일을 점령하고 콤보를 이어가는 게임. 온라인 매칭은 현재 1P vs 서버 AI 형태로 동작(PvP 전환 예정).
...
- 멀티 플레이: 로비에서 로그인/회원가입 후 자동 매칭. 줄다리기는 완전한 실시간 PvP, 땅따먹기는 임시로 서버 AI와 1P가 겨루며 전적은 저장됩니다. 성 지키기 멀티 버튼은 비활성화.
...
- 프로토콜: `docs/protocol.md`에 기본 메시지 흐름이 정리되어 있으며, LandGrab는 현재 서버가 보드 전체 스냅샷을 전송하는 형태로 구현되어 있습니다.
...
2. 로비 접속/로그인
   - 클라이언트에서 `멀티 플레이` 버튼 클릭 → 기본 Host/Port는 `127.0.0.1:7777`.
   - `회원가입`으로 ID/PW/닉네임을 한 번 등록한 뒤, `로그인`해야 게임 선택 버튼이 활성화됩니다.
3. 게임 선택
   - **줄다리기**: 실시간 PvP. 서버가 단어/아이템을 배정하며 전적이 DB에 기록됩니다.
   - **땅따먹기**: 현재는 서버 AI가 상대 역할을 하는 임시 온라인 버전입니다(전적 기록은 동일). Player2 입력 로직은 TODO.
...
- 특수 단어 확률은 코드에서 관리됩니다. 현재는 줄다리기/땅따먹기 모두 버프 15%·트랩 15%(나머지 일반) 비율을 사용합니다.

## 향후 TODO
- [ ] 땅따먹기 온라인을 실 PvP로 전환 (Player2 입력 처리, AI 제거, 프로토콜 업데이트)
- [ ] 성 지키기 멀티 모드 로비/세션 추가
- [ ] 로비 UI에서 전적/닉네임 표시 및 에러 메시지 UX 개선
