# Typing Arena 미니게임 프로젝트

## 프로젝트 개요
- 멀티플레이 감성을 살린 타자 연습 미니게임 모음집입니다.
- 현재 제공 게임
  1. **줄다리기 타자 대전 (TugOfWarGame)** – 버프/트랩 단어(70% : 30%)로 로프를 당기는 PvE 대전.
  2. **성 지키기 (CastleDefenseGame)** – 난이도별 웨이브를 타이핑으로 방어하는 디펜스 모드.
  3. **땅따먹기 Land Grab (LandGrabGame)** – 10x10 보드에서 단어를 맞춰 타일을 점령하고 콤보를 이어가는 게임.
- 모든 게임은 JavaFX 기반 UI와 Gson 기반 단어 로더(`src/main/resources/words/ko.json`)를 공유합니다.

## 개발 환경 준비
1. **JDK 21 LTS 설치**
   - `java -version`으로 21 버전이 잡히는지 확인합니다.
2. **Maven 3.9 이상 설치**
   - `mvn -v`로 설치 여부를 확인합니다.
3. **프로젝트 클론 후 의존성 받기**
   ```bash
   git clone <repo-url>
   cd JAVA-Typing-Arena
   mvn -q dependency:resolve
   ```
4. **JavaFX 플랫폼 설정**
   - `pom.xml`의 `<javafx.platform>` 값을 실제 OS(`win`, `linux`, `mac`, `mac-aarch64`)에 맞춰 수정합니다.
5. 실행 중 JavaFX NullPointerException이 발생한다면 GPU 드라이버를 업데이트하거나 `-Dprism.order=sw` VM 옵션으로 소프트웨어 렌더링을 시도하세요.

## 빌드 및 실행
### CLI
```bash
mvn clean javafx:run
```
- 기본 실행 진입점은 `typingarena.app.TypingGameApp`입니다.
- 로비 화면의 “줄다리기 게임 시작” 버튼을 누르면 `TugOfWarGame` 창이 새로 열립니다.
- 초록색 단어(버프)·빨간색 단어(트랩)·검정색 단어(일반)를 입력해 게임을 진행합니다.
- IDE에서 실행할 경우 Maven 프로젝트로 임포트한 뒤 동일한 Main 클래스를 실행하면 됩니다.

### IDE (IntelliJ 예시)
1. *Open* → `pom.xml`.
2. Maven 패널에서 `typing-arena` 프로젝트 선택.
3. Run/Debug 구성에서 `typingarena.app.TypingGameApp`을 Main Class로 지정 후 실행.
4. 실행 환경에 따라 VM 옵션:
   ```
   --module-path %PATH_TO_FX% --add-modules javafx.controls,javafx.graphics
   ```
   (`%PATH_TO_FX%`는 JavaFX SDK의 `lib` 경로)

### 멀티 플레이 실행
1. 서버 실행
   - Maven exec 플러그인으로 의존성을 포함해 실행:
     ```bash
     mvn -q exec:java -Dexec.mainClass=typingarena.server.ServerMain
     ```
   - 또는 직접 클래스패스를 지정:
     ```bash
     mvn -q clean compile
     java -cp "target/classes;%USERPROFILE%\.m2\repository\com\google\code\gson\gson\2.11.0\gson-2.11.0.jar" typingarena.server.ServerMain
     ```
     (macOS/Linux는 `:` 구분자 사용)
   기본 포트는 `7777`이며 방화벽 허용이 필요할 수 있습니다.
2. 클라이언트 로비에서 `멀티 플레이` 버튼을 눌러 매칭 창을 엽니다.
3. Host/Port/Nickname을 입력하고 `연결`을 누른 뒤, **줄다리기 (Tug of War)** 를 선택하면 자동 매칭이 진행됩니다. (다른 게임은 준비 중)
4. 두 명이 매칭되면 온라인 줄다리기 창이 열리며, 서버가 배정한 단어를 입력해 실시간 대전을 플레이할 수 있습니다.


## 디렉터리 구조
```
JAVA프로젝트/
├─ README.md                # 프로젝트 개요 및 실행 가이드
├─ pom.xml                  # Maven 빌드 설정 (Java 21, JavaFX, Gson)
├─ src/
│  ├─ main/
│  │  ├─ java/
│  │  │  └─ typingarena/
│  │  │     ├─ app/      # 엔트리 포인트, 공용 UI
│  │  │     ├─ core/     # 싱글/멀티가 공유하는 게임 코어 (예: tugofwar)
│  │  │     └─ minigames/# 각 미니게임 구현 (View + 싱글 모드)
│  │  └─ resources/
│  │        └─ words/    # ko.json(게임 사용), ko_easy|medium|hard.txt(추가 단어 예시)
└─ docs/                    # 설계 문서 및 참고 자료
```

## 코딩 컨벤션
- 패키지 명은 모두 소문자, 클래스 명은 PascalCase를 사용합니다.
- UI 텍스트는 `TypingGameApp`에서 중앙 관리하고, 미니게임 내부에서는 게임 전용 메시지만 작성합니다.
- 하드코딩된 문자열과 상수는 가능한 `static final`로 추출해 재사용합니다.
- 아이템/단어 비율 등의 밸런스 값을 조정할 때는 `GameLogic`의 관련 상수를 수정하고 README를 업데이트해 주세요.
- PR 작성 시 변경 요약과 테스트 결과(수동 테스트 포함)를 간단히 남겨 주세요.

## 단어 리스트 커스터마이징
- `src/main/resources/words/ko.json` 안의 `"words"` 배열에 단어를 추가하면 바로 게임에 반영됩니다.
- JSON 형식이므로 주석은 사용할 수 없습니다. 설명을 남기고 싶다면 README나 별도 문서를 활용하세요.
- 난이도별 단어 예시는 `ko_easy.txt`, `ko_medium.txt`, `ko_hard.txt`에 정리돼 있으며 필요 시 JSON으로 변환해 사용하세요.

## 향후 TODO
- [ ] 로비 화면에서 다중 미니게임 선택 UI 제공
- [ ] 버프/트랩 아이템 종류 확장 및 시각 효과 강화
- [ ] 멀티플레이용 네트워크 레이어 설계 (`docs/` 하위에 문서 추가)
- [ ] 공용 설정/리소스 로더 클래스 설계
- [ ] 빌드/테스트 자동화를 위한 스크립트(`scripts/`) 마련
- [ ] README에 실행 GIF 또는 스크린샷 첨부

# 🕹️ 미니게임: 땅따먹기 (Land Grab)
Typing Arena의 실시간 1:1 타자 대결 미니게임입니다. 제한 시간 내에 AI보다 더 많은 타일을 점령하세요!

## 🏁 게임 목표
제한 시간: 60초

승리 조건: 60초가 지났을 때, AI보다 더 많은 타일(칸)을 차지하고 있으면 승리합니다.

게임 보드: 10x10 (총 100칸)의 그리드에서 AI와 실시간으로 땅을 뺏고 뺏깁니다.

## ⌨️ 기본 규칙: 땅 뺏기 메커니즘
타일은 총 3가지 상태로 존재하며, 어떤 상태의 타일을 입력하느냐에 따라 결과가 다릅니다.

**⬜ 빈 땅 (흰색 타일)**

단어를 정확히 입력하면 내 땅 (파란색)이 됩니다.

(결과: 내 점수 +1)

**🟥 AI 땅 (빨간색 타일)**

단어를 정확히 입력하면 빈 땅 (흰색)으로 되돌아갑니다. (땅을 빼앗아 중립화시킵니다.)

(결과: AI 점수 -1)

**🟦 내 땅 (파란색 타일)**

이미 점령한 땅입니다.

(결과: 단어를 입력해도 아무 효과가 없습니다.)

**🤖 AI의 움직임**

AI는 약 2초마다 한 번씩 타일을 점령합니다.

AI는 빈 땅을 우선적으로 점령하며, 빈 땅이 없다면 플레이어의 땅을 공격하여 빈 땅으로 되돌립니다.

## ⚡️ 특수 단어 (아이템)
타일 위의 단어는 일정 확률로 '버프' 또는 '트랩' 효과를 가집니다.

**<font color="#0064C8">💎 버프 (파란색 단어): 스플래시!</font>**

효과: 단어 입력 성공 시, 해당 타일을 점령함과 동시에 상하좌우 인접한 타일 중 하나를 무작위로 추가 공격/점령합니다.

시각 효과: 단어를 맞춘 위치에서 "스플래시!" 텍스트가 떠오르는 애니메이션이 재생됩니다.

**<font color="#D04444">🐙 트랩 (빨간색 단어): 먹물!</font>**

효과: 단어 입력 성공 시, 해당 타일을 점령하지만 3초간 화면이 어두워지는 '먹물' 효과가 발동됩니다.

시각 효과: "먹물!" 텍스트와 함께 3초간 게임 화면이 어두워져 단어 식별이 어려워집니다.

## 🏆 게임 종료 및 승패 판정
다음 두 가지 조건 중 하나가 만족되면 게임이 즉시 종료됩니다.

시간 초과: 60초의 제한 시간이 모두 지났을 때

전장 점령: 100칸의 모든 타일이 '내 땅' 또는 'AI 땅'으로 채워졌을 때

게임 종료 시점의 **'내 점수(파란색 칸 수)'**와 **'AI 점수(빨간색 칸 수)'**를 비교하여 승패가 결정됩니다.

## 🗓️ 향후 업데이트 계획 (To-Do List)
현재 버전은 싱글 플레이를 기준으로 하며, 'Typing Arena' 프로젝트의 최종 목표를 위해 다음과 같은 기능들이 추가될 예정입니다.

**[핵심] 멀티플레이 전환**

현재의 AI 로직을 실제 상대 플레이어(Player 2)로 교체합니다.

서버-클라이언트 간 실시간 타일 상태 동기화를 위한 네트워크 레이어를 설계합니다.

**아이템 및 효과 확장**

'줄다리기' 게임의 '파워그립', '앵커' 아이템을 땅따먹기에 맞게 도입하거나 새로운 아이템을 추가합니다. (예: 일정 시간 동안 내 땅을 보호하는 '방패', 상대방의 입력을 반대로 하는 '혼란' 등)

아이템 밸런스를 조정하고 시각 효과를 강화합니다.

**UI/UX 폴리싱**

게임 폰트, 색상, 애니메이션 효과를 더 세련되게 다듬습니다.

게임 시작 전 로비 화면에 '땅따먹기' 미니게임을 선택하는 버튼을 추가합니다.

**공용 모듈 설계**

'줄다리기'와 '땅따먹기'에서 공통으로 사용하는 단어 로더, 아이템 로직, UI 컴포넌트를 분리하여 공용 라이브러리로 만듭니다.

궁금한 사항이나 제안이 있으면 PR 또는 이슈를 통해 공유해 주세요. 함께 프로젝트를 발전시켜 봅시다! 🙌
