...
import typingarena.minigames.castledefense.CastleDefenseOnlineStage; // [중요] 임포트 확인!
...
public class MultiLobbyPane extends BorderPane {
    ...
    private final Button castleBtn = new Button("성 지키기 (Castle Defense)");
    ...
    // [추가] 성 지키기 스테이지 변수 추가
    private CastleDefenseOnlineStage castleStage;
    ...
    private VBox buildCenter() {
        ...
        castleBtn.setText("🛡 타자 디펜스 (Typing Defense)");
    ...
    private void handleServerMessage(Message msg) {
        ...
        Platform.runLater(() -> {
            switch (type) {
                ...
                case "MONSTER_KILLED" -> forwardToGames(msg);
                case "MONSTER_UPDATE" -> forwardToGames(msg); // [추가] 단어 변경
                case "OPPONENT_ATTACK" -> forwardToGames(msg); // [추가] 공격 모션      
                case "GAME_REMATCH_NOTICE" -> forwardToGames(msg);
    ...
    private void forwardToGames(Message msg) {
        ...
        // [핵심 수정] 성 지키기 스테이지로 메시지 전달 추가
        if (castleStage != null) castleStage.handleMessage(msg);
    }
    ...
    private void handleGameStart(Message msg) {
        ...
        if ("TUG_OF_WAR".equalsIgnoreCase(gameType)) {
            ...
            // [수정] castleStage 변수에 저장하여 forwardToGames에서 쓸 수 있게 함
            if (castleStage == null) castleStage = new CastleDefenseOnlineStage(client);
            castleStage.handleMessage(msg);
            castleStage.show();
        } else {
